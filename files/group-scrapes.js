...
export function groupScrapes(scrapes) {
    const grouped = scrapes.reduce((acc, scrape) => {
        const list = findLongestSelector(scrape.elements);
        acc.push(Object.assign(Object.assign({}, scrape), { list, groups: [] }));
        return acc;
    }, []);
    const output = mergeScrapes(grouped);
}
function mergeScrapes(grouped) {
    let lists = [];
    const groups = [];
...
function createGroups(first, second, groups, boundaries) {
    const output = [];
...
function findLongestSelector(elements) {
    if (elements.length > 1) {
        ...
        for (let i = 0; i < elements.length - 1; i++) {
            ...
            for (let j = i + 1; j < elements.length; j++) {
                ...
                selectors.push(createSelector(findFirstCommonAncestor(first, second), { isMulti: true }));
            }
        }
